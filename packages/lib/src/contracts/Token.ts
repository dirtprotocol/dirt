...
export class TokenValue {
  ...
  add(value: number|TokenValue): TokenValue {
    const token =
        (value instanceof TokenValue) ? value : TokenValue.from(value);
...
/**
 * Convenience class for `ProtocolToken.sol`.
 */
export class Token extends Contract {
  ...
  async init(): Promise<void> {
    ...
    this.totalSupply =
        TokenValue.fromRaw(await this.instance.totalSupply.call());
  }
  ...
  async transfer(toAddress: string, amount: TokenValue): Promise<void> {
    ...
    await this.instance.transfer(
        toAddress, amount.raw, {from: this.dirt.defaultAccount()});
  }

  async balanceOf(account?: string): Promise<TokenValue> {
    account = account || this.dirt.defaultAccount();
    const raw = await this.instance.balanceOf.call(
        account, {from: this.dirt.defaultAccount()});
    return TokenValue.fromRaw(raw);
  }

  async approveAndCall(
      toAddress: string, amount: TokenValue, extraData: string): Promise<void> {
    return this.instance.approveAndCall(toAddress, amount.raw, extraData);
  }

  async approveFor(spender: string, value: TokenValue): Promise<void> {
    this.trace.message(`Approve ${value.value} for ${spender}`);
    await this.instance.approve(
        spender, value.raw, {from: this.dirt.defaultAccount()});
  }

  async allowance(owner: string, spender: string): Promise<TokenValue> {
    const raw = await this.instance.allowance.call(owner, spender);
    return TokenValue.fromRaw(raw);
  }

  async increaseApproval(spender: string, value: TokenValue): Promise<void> {
    const currentAllowance =
        await this.allowance(this.dirt.defaultAccount(), spender);
    const increased = currentAllowance.add(value);
    this.trace.message(`Increasing approval from ${currentAllowance.value} to ${
        increased.value} for ${spender}`);
    await this.instance.approve(
        spender, increased.raw, {from: this.dirt.defaultAccount()});
  }

  async decreaseApproval(spender: string, value: TokenValue): Promise<void> {
    const currentAllowance =
        await this.allowance(this.dirt.defaultAccount(), spender);
    const decreased = currentAllowance.sub(value);
    this.trace.message(`Decreasing approval from ${currentAllowance.value} to ${
        decreased.value} for ${spender}`);
    await this.instance.approve(
        spender, decreased.raw, {from: this.dirt.defaultAccount()});
  }

  async ensureSpenderApproval(spender: string, required: TokenValue):
      Promise<TokenSpenderApprovalScope> {
    const current = await this.allowance(this.dirt.defaultAccount(), spender);
    ...
    const additional = required.sub(current);

    await this.increaseApproval(spender, additional);

    return new TokenSpenderApprovalScope(
        () => this.decreaseApproval(spender, additional));
  }
}
