const Registry = artifacts.require("TestChallengeableRegistryLockedCommit");
const Utils = require('../util/contract-utils');
const TokenValue = require('@dirt/lib').TokenValue;

contract('LockedCommitRevealVoteController', (accounts) => {
    ...
    it("Allows challenges to be created", async () => {
        ...
        const vote0 = await users[0].registry.getVoteInstance(key);

        let status = await vote0.getStatus();

        assert.equal(users[0].registry.voteStyle, "LOCKED_COMMIT_REVEAL");
        assert.equal(vote0.style, "LOCKED_COMMIT_REVEAL");
    ...
    it("Allows users to commit votes", async () => {
        ...
        await vote0.commit(1, vote0Salt, TokenValue.from(1), TokenValue.from(1));

        // Confirm 1 + 1 is taken
        let balance0After = await users[0].dirt.Token.balanceOf();

        let expected = balance0Before.value - 2;

        assert.equal(balance0After.value, expected, "Stake and Commit Stake taken from user");
    });
    ...
    it("Allows users to reveal votes", async () => {
        ...
        let expected = balance0BeforeReveal.value + 1;
